...
class QueryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $query = new Query();
        $result = $query->select()->from('post')->execute(ResultDataDecorator::RESULT_ENTITY);
    ...
    {
        $query = new Query();
        $result = $query->select()->from('post')->where('id = 1')->execute(ResultDataDecorator::RESULT_COLLECTION);
